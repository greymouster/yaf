...
class TZ_Request
{
     ...
     */
    private function inject_check($inp)
    {
        if (is_array($inp)) {
        ...
        if (!empty($inp) && is_string($inp)) {
            ...
            $check = preg_match('/select|insert|update|delete|\'|\\*|\*|\.\.\/|\.\/|union|into|load_file|outfile/i', $inp);
            if ($check) {
                TZ_Response::error(108, "系统警告：请不要在参数中包含非法字符尝试注入！");
            }
